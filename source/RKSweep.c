...
static void *sweepManager(void *in) {
    ...
    if (!engine->doNotWrite && engine->hasHandleFilesScript) {
        ...
        if (strlen(engine->handleFilesScriptArchivedFileExtension)) {
            RKReplaceFileExtension(filename, ".__", engine->handleFilesScriptArchivedFileExtension);
...
static void *rayGatherer(void *in) {
    ...
    if (strlen(engine->handleFilesScript)) {
        RKLog(">%s Handle files using '%s%s%s'   %s%s%s\n", engine->name,
              rkGlobalParameters.showColor ? "\033[3;4m" : "",
              engine->handleFilesScript,
              rkGlobalParameters.showColor ? "\033[23;24m" : "",
              strlen(engine->handleFilesScriptArchivedFileExtension) ? " --> " : "",
              engine->handleFilesScriptArchivedFileExtension);
    }

...
static void *sweepWriter(void *in) {
    ...
    if (engine->verbose > 1) {
        RKLog("%s C%02d-%02d-%02d   E%5.2f/%5.2f-%5.2f   A%6.2f-%6.2f   M%03x-%03x-%03x   (%d x %d, %.1f km)\n",
              ...
              n, S->header.gateCount, 1.0e-3f * S->header.gateCount * S->header.gateSizeMeters);
    }
    ...
    if (engine->verbose) {
        RKLog("%s C%02d   E%5.2f/%5.2f-%5.2f   A%6.2f-%6.2f   M%03x-%03x   (%s%d%s x %d, %.1f km)\n",
              engine->name,
              ...
              n,
              RKNoColor,
			  S->header.gateCount, 1.0e-3f * S->header.gateCount * S->header.gateSizeMeters);
    }

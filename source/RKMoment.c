    ...
    while (engine->state & RKEngineStateActive) {
        ...
        if (S->header.s & RKPulseStatusCompressed) {
            ...
            if (ic != S->header.configIndex) {
                ...
                RKFloat r = 0.0f;
//                printf("---%s  gateCount = %d %d  %.2f\n", name, config->filterAnchors[0].outputOrigin, config->filterAnchors[0].maxDataLength, config->ZCal[0][0]);
//                printf("---%s  gateCount = %d %d  %.2f\n", name, config->filterAnchors[1].outputOrigin, config->filterAnchors[1].maxDataLength, config->ZCal[0][1]);
...
//                }
                for (k = 0; k < config->filterCount; k++) {
                    for (i = config->filterAnchors[k].outputOrigin; i < config->filterAnchors[k].maxDataLength; i++) {
                        r = (RKFloat)i * gateSizeMeters;
                        space->rcor[0][i] = 20.0f * log10f(r) - 30.0f + config->ZCal[0][k];
                        space->rcor[1][i] = 20.0f * log10f(r) - 30.0f + config->ZCal[1][k];
                    }
                }

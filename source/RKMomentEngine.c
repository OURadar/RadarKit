...
static void *momentEngineCore(void *in) {
    ...
    while (engine->state & RKEngineStateWantActive) {
        ...
        if (path.length > 3 && deltaAzimuth < 3.0f && deltaElevation < 3.0f && pulses[0]->header.s & RKPulseStatusCompressed) {
            ...
            zeroOutRay(ray);
            if (engine->verbose > 1) {
                RKLog("%s Skipped a ray with %d sample%s   deltaAz = %.2f   deltaEl = %.2f   pulses[0]->header.s = 0x%x\n", me->name,
...
static void *pulseGathererV1(void *_in) {
    ...
    while (engine->state & RKEngineStateWantActive) {
        ...
        s = 0;
        while ((pulse->header.s & RKPulseStatusReadyForMomentEngine) != RKPulseStatusReadyForMomentEngine && engine->state & RKEngineStateWantActive) {
            usleep(1000);
            if (++s % 200 == 0 && engine->verbose > 1) {
                RKLog("%s sleep 2/%.1f s   k = %d   pulseIndex = %d   header.s = 0x%02x\n",
                      engine->name, (float)s * 0.001f, k , *engine->pulseIndex, pulse->header.s);

...
classdef iqread
    ...
    methods
        ...
        function self = iqread(filename, maxPulse)
            ...
            if self.header.buildNo == 0
                frewind(fid)
                self.constants
                self.header.preface = deblank(fread(fid, [1 256], 'char=>char'));
                self.header.buildNo = fread(fid, 1, 'uint32');
            end
            ...
            elseif self.header.buildNo == 1
                h = memmapfile(self.filename, ...
                    'Offset', 256 + 4, ...    % RKName * (char) + (uint32_t)
                    'Repeat', 1, ...
                    'Format', { ...
                        'uint32', [1 1], 'initFlags'; ...
                        'uint32', [1 1], 'pulseCapacity'; ...
            ...
            if self.header.buildNo == 8
                ...
                c = memmapfile(self.filename, ...
                    ...
                    'Format', { ...
                        'uint64', [1 1], 'i'; ...
                        'uint64', [1 1], 'volumeIndex'; ...
                        'uint64', [1 1], 'sweepIndex'; ...
                        ...
                        'uint32', [1 1], 'transitionGateCount'; ...
                        'uint32', [1 1], 'ringFilterGateCount'; ...
                        ...
                        'uint8',  [1 self.constants.RKName], 'waveformName'; ...
                        'uint64', [1 3], 'trash'; ...
                        'uint8',  [1 1], 'momentMethod'; ...
                        'uint32', [1 8], 'userIntegerParameters'; ...
                        'single', [1 8], 'userFloatParameters'; ...
                        'uint8',  [1 480], 'vcpDefinition'; ...
                        });
                config = c.data;
                config.waveformName = deblank(char(config.waveformName));
                config.vcpDefinition = deblank(char(config.vcpDefinition));
                self.header.config = rmfield(config, 'trash');
                ...
                % Waveform contains only dat.header.waveform.filterCounts(1), revise config.pw here
                self.header.config.pw = self.header.config.pw(self.header.waveform.filterCounts(1));
                self.header.config.prt = self.header.config.prt(self.header.config.prt > 0);
            elseif self.header.buildNo == 6 || self.header.buildNo == 7
                % (RKRadarDescOffset = 256) + (RKRadarDesc) --> RKConfig
                offset = self.constants.RKRadarDescOffset + self.constants.RKRadarDesc;
                %  Read in RKConfig
                c = memmapfile(self.filename, ...
            ...
            elseif self.header.buildNo == 5
                ...
                self.header.dataType = fread(fid, 1, 'uint8');
                offset = self.constants.RKFileHeader;
                % Waveform contains only self.header.config.filterCount, revise config.pw here
                self.header.config.pw = self.header.config.pw(self.header.config.filterCount);
                self.header.config.prt = self.header.config.prt(self.header.config.prt > 0);
            ...
            elseif self.header.buildNo == 1
                c = memmapfile(self.filename, ...
                    'Offset', 256 + 4 + 1604, ... % RKName * (char) + (uint32_t) + (RKRadarDesc)
                    'Repeat', 1, ...
                    'Format', { ...
                        'uint64', [1 1], 'i'; ...
                        'uint32', [1 self.constants.RKMaxFilterCount], 'pw'; ...
                ...
                self.header.config.prf = self.header.config.prf(self.header.config.prf > 0);
                self.header.config.prt = 1.0 / single(self.header.config.prf);
                self.header.dataType = 1;
                offset = self.constants.RKFileHeader;
            end
            ...
            if self.header.buildNo > 5
                ...
                downSampledGateCount = gateCount;
            end
            fclose(fid);
            fprintf('capacity = %d   gateCount = %d   downSampledGateCount = %d\n', capacity, gateCount, downSampledGateCount);
            ...
            % Pulses
            if self.header.buildNo == 7 || self.header.buildNo == 8
            ...
            elseif self.header.buildNo > 1
                ...
                if self.header.dataType ~= 1
                    fprintf('Error. buildNo = 1 should only support raw.\n')
                    return
                end
            m = memmapfile(self.filename, ...
                'Offset', self.constants.RKFileHeader, ...
